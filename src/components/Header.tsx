    ...
    <header className="sticky top-0 z-30 border-b border-brand-100/60 bg-white/80 backdrop-blur-lg">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-5 py-2">
        <Link href="/" className="transition hover:opacity-80">
          <Image src="/logo.svg" alt="Bhuvika Studio" width={200} height={50} className="h-[52px] w-auto" priority />
        </Link>
        <nav className="hidden items-center gap-6 text-sm font-semibold text-brand-800 md:flex">
          <Link href="/shop" className="nav-link transition hover:text-brand-500">Shop</Link>
          <Link href="/categories" className="nav-link transition hover:text-brand-500">Categories</Link>
          <Link href="/checkout" className="nav-link transition hover:text-brand-500">Checkout</Link>
          <Link href="/admin" className="nav-link flex items-center gap-1 transition hover:text-brand-500">
